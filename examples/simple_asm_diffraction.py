...
R0 = 0.8e-4  # Radius of the circular aperture (meters).
U0 = circular_aperture(X, Y, R0).astype(np.complex128)

Uz = asm_propagator(U0, (X, Y), z=z, λ=λ, n=n, include_evanescent=False)
...
ax[1].set_title(f"ASM propagated intensity (z={z} m)")
ax[1].set_xlabel("x [m]")
ax[1].set_ylabel("y [m]")
