...
def main() -> None:
    ...
    native = field.propagate(z_focus, method="asm", pad_factor=2)
    ...
    native.plot(
        ax[0], title=f"Native ASM at focus (dx = {dx_in * 1e6:.1f} um)", vmin=-4.0
    )
    ax[0].set_xlim(-ZOOM_HALF, ZOOM_HALF)
    ax[0].set_ylim(-ZOOM_HALF, ZOOM_HALF)
    zoom.plot(
        ax[1], title=f"Decoupled output grid (dx = {dx_out * 1e9:.0f} nm)", vmin=-4.0
    )
