...
half-planes), and `PolychromaticField` runs the per-wavelength
...
def main() -> None:
    ...
    img = (
        PolychromaticField(
            grid, 1.0, wavelengths=wavelengths, weights=d65_weights(wavelengths * 1e9)
        )
        .add_aperture(polygon_aperture, 6, CIRCUMRADIUS, rotation=np.pi / 6, antialiased=True)
        .propagate(
            Z,
            method="asm",
            pad_factor=2,
    ...
    img.plot(ax, title=f"Hexagon in white light (D65, z = {Z} m)")
