...
def grating_field(grid):
    x, y = grid
    mask = cross_grating(x, y, PERIOD, kind="ronchi", duty=0.5) * square_aperture(x, y, BEAM_HALF)
...
def main() -> None:
    ...
    plot_intensity(ax[0], U0, title=f"Square (cross) grating, d = {PERIOD*1e6:.0f} µm")
